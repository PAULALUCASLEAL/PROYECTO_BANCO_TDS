...
# Especificación detallada del diagrama UML de clases 
...
## 2. Qué clases deben aparecer

En el diagrama deben aparecer las siguientes clases:
...
## 3. Qué clases NO se deben meter
...
# 15. Orden recomendado para dibujarlo

Para que quede limpio, se recomienda seguir este orden:

1. En el centro, dibujar el paquete `domain`.
2. Dentro de `domain`, colocar:
   - `Cuenta` a la izquierda;
   - `Gasto` a la derecha de `Cuenta`;
   - `AlertaGasto` debajo de `Cuenta`;
   - `Notificacion` debajo de `Gasto`;
   - `Periodo` al lado de `AlertaGasto` y `Notificacion`.
3. Dibujar las relaciones principales:
   - `Cuenta 1 -- 0..* Gasto`;
   - `AlertaGasto 1 -- 0..* Notificacion`;
   - flechas hacia `Periodo`.
4. Encima del dominio, colocar el paquete `application` con los casos de uso.
5. A la derecha del dominio, colocar el paquete `repo` con los repositorios.
6. A la izquierda del dominio, colocar el paquete `filtros`.
7. Debajo del dominio, colocar `service` con `AlertaService`.
8. Añadir dependencias con líneas discontinuas para los casos de uso y servicios.
9. Añadir agregaciones desde repositorios hacia las entidades.
10. Revisar que no haya flechas cruzándose demasiado.

---

# 16. Leyenda UML que debe usarse
